...
func tableSlackGroup() *plugin.Table {
	return &plugin.Table{
		...
		Columns: []*plugin.Column{
			...
			{Name: "date_create", Type: proto.ColumnType_DATETIME, Transform: transform.FromField("DateCreate").Transform(jsonTimeToTime), Description: "Date when the group was created."},
			{Name: "date_update", Type: proto.ColumnType_DATETIME, Transform: transform.FromField("DateUpdate").Transform(jsonTimeToTime), Description: "Date when the group was last updated."},
			{Name: "date_delete", Type: proto.ColumnType_DATETIME, Transform: transform.FromField("DateDelete").Transform(jsonTimeToTime), Description: "Date when the group was deleted."},
